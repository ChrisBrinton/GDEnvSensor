...
void processMessage(packet_t * data) {
    ...
    for (uint8_t j=0;j<data->pairCount;j++){
      //TODO: Replace with linked list code to hold node data for the display.
      ...
      for (unsigned int i=0; i<count; i++) {
          if ((getSetting("nodeid" + String(i)) == String(data->senderID)) &&
              (getSetting("key" + String(i)) == data->pairs[j].name)) {
              mqttSend((char *) getSetting("topic" + String(i)).c_str(), (char *) String(data->pairs[j].value).c_str());
              found = true;
              tmpTopic = String(getSetting("topic" + String(i)).c_str()) + String("RSSI/");
              break;
          }
      }
      ...
      if (!found) {
          ...
          if (topic.length() > 0) {
              ...
              topic.replace("{key}", String(data->pairs[j].name));
              String tmpHN = String(getSetting("hostname", APP_NAME));
              tmpHN.replace("_","/");
              topic.replace("{hostname}", tmpHN);
              topic += "/";
              mqttSend((char *) topic.c_str(), (char *) String(data->pairs[j].value).c_str());
              tmpTopic = getSetting("defaultTopic", MQTT_DEFAULT_TOPIC);
              tmpTopic.replace("{nodeid}", String(data->senderID));
              tmpTopic.replace("{key}", String("RSSI"));
              tmpTopic.replace("{hostname}", tmpHN);
              tmpTopic += "/";

          }
      }
    }
    mqttSend((char *) tmpTopic.c_str(), (char *) String(data->rssi).c_str());
}
